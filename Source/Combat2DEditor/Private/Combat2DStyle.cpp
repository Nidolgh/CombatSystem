...
#define IMAGE_PLUGIN_BRUSH( RelativePath, ... ) FSlateImageBrush( FCombat2DStyle::InContent( RelativePath, ".png" ), __VA_ARGS__ )
#define IMAGE_BRUSH(RelativePath, ...) FSlateImageBrush(StyleSet->RootToContentDir(RelativePath, TEXT(".png")), __VA_ARGS__)
#define BOX_BRUSH(RelativePath, ...) FSlateBoxBrush(StyleSet->RootToContentDir(RelativePath, TEXT(".png")), __VA_ARGS__)
#define DEFAULT_FONT(...) FCoreStyle::GetDefaultFontStyle(__VA_ARGS__)

FString FCombat2DStyle::InContent(const FString& RelativePath, const ANSICHAR* Extension)
{
	static FString ContentDir = IPluginManager::Get().FindPlugin(TEXT("Paper2D"))->GetContentDir();
	return (ContentDir / RelativePath) + Extension;
}

TSharedPtr< FSlateStyleSet > FCombat2DStyle::StyleSet = nullptr;
TSharedPtr< class ISlateStyle > FCombat2DStyle::Get() { return StyleSet; }

FName FCombat2DStyle::GetStyleSetName()
{
	static FName StyleName(TEXT("Combat2DStyle"));
	return StyleName;
}

BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION

void FCombat2DStyle::Initialize()
{
	// Const icon sizes
