...
#include "SCombatFlipbookTrackHandle.h"
...
{
	const FCombatFlipbookEditorCommands& Commands = FCombatFlipbookEditorCommands::Get();
...
void SFlipbookKeyframeWidget::Construct(const FArguments& InArgs, int32 InFrameIndex, TSharedPtr<FUICommandList> InCommandList)
{
	...
	ChildSlot
		...
		[
			SNew(SBorder)
			.BorderImage(FCombat2DStyle::Get()->GetBrush("CombatFlipbookEditor.RegionBody"))
...
TSharedPtr<SWidget> FFlipbookKeyFrameDragDropOp::GetDefaultDecorator() const
{
	...
	return SNew(SBox)
		...
		[
			SNew(SBorder)
			.BorderImage(FCombat2DStyle::Get()->GetBrush("FlipbookEditor.RegionBody"))
...
void SFlipbookTimelineTrack::Rebuild()
{
	...
	if (UPaperFlipbook* Flipbook = CombatFlipbookBeingEdited.Get()->TargetFlipbook)
	{
		for (int32 KeyFrameIdx = 0; KeyFrameIdx < Flipbook->GetNumKeyFrames(); ++KeyFrameIdx)
		{
			MainBoxPtr->AddSlot()
				...
				.FlipbookBeingEdited(this->CombatFlipbookBeingEdited.Get()->TargetFlipbook)
