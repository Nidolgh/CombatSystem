...
FCombatSpriteGeometryEditMode::FCombatSpriteGeometryEditMode()
	...
	, GeometryVertexColorHitbox(FLinearColor::Red)
	, GeometryVertexColorHurtbox(FLinearColor::Yellow)
	...
	, CollDataToEditIndex(-1)
	, PreviousCollDataToEditIndex(-1)
{
...
void FCombatSpriteGeometryEditMode::Render(const FSceneView* View, FViewport* Viewport, FPrimitiveDrawInterface* PDI)
{
	...
	if (FrameCollDataArray != nullptr && FrameCollDataArray->IsValidIndex(CollDataToEditIndex))
	{
		for (size_t i = 0; i < FrameCollDataArray->Num(); i++)
		{
			...
			SetActiveGeometryColor(FrameInstruction->CollisionType, (CollDataToEditIndex == i));
				
			SpriteGeometryHelper.DrawGeometry(*View, *PDI, GeometryVertexColorActive, NegativeGeometryVertexColor);
		}
	}
}
...
bool FCombatSpriteGeometryEditMode::HandleClick(FEditorViewportClient* InViewportClient, HHitProxy* HitProxy, const FViewportClick& Click)
{
	...
	if (bAllowSelectVertex && (SelectedItemProxy != nullptr) && SelectedItemProxy->Data != nullptr)
	{
	...
	//{
	//	const bool bUserWantsPaint = bIsLeftButtonDown && ( !GetDefault<ULevelEditorViewportSettings>()->bLeftMouseDragMovesCamera ||  bIsCtrlKeyDown );
	//	//findme
...
void FCombatSpriteGeometryEditMode::Tick(FEditorViewportClient* ViewportClient, float DeltaTime)
{
	...
	// Everytime we detect and edit in current collision data to be edit we clear the selection
	if (PreviousCollDataToEditIndex != CollDataToEditIndex)
	{
		PreviousCollDataToEditIndex = CollDataToEditIndex;
...
void FCombatSpriteGeometryEditMode::SetCollDataToEditIndex(int32 newCollDataToEditIndex)
{
	CollDataToEditIndex = newCollDataToEditIndex;
}

int32 FCombatSpriteGeometryEditMode::GetCollDataToEditIndex() const
{
	return CollDataToEditIndex;
}
...
{	
	...
	case ECollisionType::HitBox:
		GeometryVertexColorActive = GeometryVertexColorHitbox;
		break;
	case ECollisionType::HurtBox: 
		GeometryVertexColorActive = GeometryVertexColorHurtbox;
		break;
	default: 
